...
class PriceMonitoringTask:
    ...
    async def convert_timed_out_signal_entry_orders(
        ...
        logging.info(
            f"⏰ 发现 {len(timed_out_orders)} 个超时 signal_entry 限价单，开始尝试受限补单"
        )
        for order in timed_out_orders:
            try:
                ...
                logging.error(
                    f"❌ 超时 signal_entry 受限补单异常: account_id={order.get('account_id')}, order_id={order.get('order_id')}, err={e}",
                    exc_info=True,
                )
    ...
    async def _convert_single_timed_out_signal_entry_order(self, order: dict):
        ...
        try:
            ...
            if not market_price or Decimal(str(market_price)) <= 0:
                logging.error(
                    f"❌ 超时单受限补单失败，无法获取市价: account_id={account_id}, order_id={order_id}, symbol={symbol}"
                )
                return

            signal_price = Decimal(str((signal or {}).get("price") or order.get("price") or 0))
            if signal_price <= 0:
                logging.error(
                    f"❌ 超时单缺少有效信号价，拒绝补单: account_id={account_id}, signal_id={signal_id}, order_id={order_id}"
                )
                return

            symbol_config = await self.db.get_config_by_account_and_symbol(account_id, symbol)
            raw_slippage = (symbol_config or {}).get("timeout_entry_max_slippage_pct", "0.002")
            try:
                max_slippage = Decimal(str(raw_slippage))
            except Exception:
            ...
            price_cap = signal_price * (Decimal("1") + max_slippage) if is_buy else signal_price * (Decimal("1") - max_slippage)
            current_price = Decimal(str(market_price))
            exceeds_cap = current_price > price_cap if is_buy else current_price < price_cap
            if exceeds_cap:
                logging.warning(
                    f"🚫 超时入场已超滑点上限，放弃追单: account_id={account_id}, signal_id={signal_id}, "
                    f"side={order['side']}, signal_price={signal_price}, market_price={current_price}, "
                    f"limit={price_cap}, max_slippage={max_slippage}"
                )
                return

            # IOC 限价只会按 cap 或更优价格立即成交；不能成交会被交易所自动取消。
            limit_price = Decimal(str(exchange.price_to_precision(symbol, float(price_cap))))

            client_order_id = await get_client_order_id()
            replacement_order = await open_position(
                self,
                account_id,
                symbol,
                order["side"],
                ...
                extra_params={"ordType": "ioc"},
            )
            if not replacement_order:
                logging.error(
                    f"❌ 超时单撤单后 IOC 补单失败: account_id={account_id}, order_id={order_id}, symbol={symbol}"
                )
                return

            replacement_state = (
                (replacement_order.get("info", {}) or {}).get("state")
                or replacement_order.get("status")
                or "canceled"
            )
            await self.db.add_order(
                {
                    ...
                    "symbol": symbol,
                    "signal_id": signal_id,
                    "order_source": "signal_entry_timeout_ioc",
                    ...
                    "order_type": "limit",
                    "side": order["side"],
                    "status": replacement_state,
                    "position_group_id": order.get("position_group_id") or "",
                }
            )
            logging.info(
                f"✅ 超时 signal_entry 已改为 IOC 限价补单: account_id={account_id}, signal_id={signal_id}, old_order={order_id}, new_order={replacement_order.get('id')}, limit={limit_price}"
            )
        finally:
            if exchange:
                try:
