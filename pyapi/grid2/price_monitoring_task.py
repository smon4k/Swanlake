...
class PriceMonitoringTask:
    ...
    async def update_order_status(self, order: dict, account_id: int, executed_price: float = None, fill_date_time: str = None):
        ...
        print(f"最新成交价格: {executed_price}")
        if get_order_by_price_diff:
            if order['side'] == 'sell' and (Decimal(executed_price) > Decimal(get_order_by_price_diff['price'])):
            # if order['side'] == 'buy':
                # 计算利润
                group_id = str(uuid.uuid4())
                profit = (Decimal(executed_price) - Decimal(get_order_by_price_diff['price'])) * Decimal(get_order_by_price_diff['quantity'])
                print(f"配对订单成交，利润 buy: {profit}")
                logging.info(f"配对订单成交，利润 buy: {profit}")
            if order['side'] == 'buy' and (Decimal(executed_price) < Decimal(get_order_by_price_diff['price'])):
            # if order['side'] == 'sell':
                # 计算利润
                group_id = str(uuid.uuid4())
