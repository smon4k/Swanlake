...
CREATE TABLE IF NOT EXISTS `g_accounts` (
  ...
  `trade_status` enum('normal','blocked') COLLATE utf8_unicode_ci NOT NULL DEFAULT 'normal' COMMENT '交易状态 normal:正常 blocked:冻结',
  `trade_block_reason` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL COMMENT '冻结原因',
  `trade_blocked_at` datetime DEFAULT NULL COMMENT '冻结时间'
...
CREATE TABLE IF NOT EXISTS `g_signals` (
  ...
  `sl` decimal(18,8) DEFAULT NULL COMMENT '自定义止损价',
  `tp` decimal(18,8) DEFAULT NULL COMMENT '自定义止盈价',
  ...
  `status` enum('pending','processing','partial','processed','failed') COLLATE utf8_unicode_ci DEFAULT 'pending',
...
CREATE TABLE IF NOT EXISTS `g_signal_recovery_tasks` (
  ...
  `signal_sl` decimal(18,8) DEFAULT NULL COMMENT '信号自定义止损价',
  `signal_tp` decimal(18,8) DEFAULT NULL COMMENT '信号自定义止盈价',
...
--
-- Indexes for table `g_signal_recovery_tasks`
--
ALTER TABLE `g_signal_recovery_tasks`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uniq_signal_account` (`signal_id`,`account_id`),
  ADD KEY `idx_status_next_retry` (`status`,`next_retry_at`),
  ADD KEY `idx_signal_status` (`signal_id`,`status`),
...
--
-- Indexes for table `g_trade_symbols`
--
ALTER TABLE `g_trade_symbols`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uniq_symbol` (`symbol`),
  ADD UNIQUE KEY `uniq_swap_symbol` (`swap_symbol`),
...
ALTER TABLE `g_signals`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
--
-- AUTO_INCREMENT for table `g_signal_recovery_tasks`
--
ALTER TABLE `g_signal_recovery_tasks`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
--
...
--
-- AUTO_INCREMENT for table `g_trade_symbols`
--
ALTER TABLE `g_trade_symbols`
