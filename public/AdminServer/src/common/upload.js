...
let clientOSS;
const appServer = "/api/files/upload"
    // = new OSS({
    //   region: 'oss-cn-beijing',
    //   accessKeyId: 'LTAIhrkdcjUVkZyV',
    //   accessKeySecret: 'CuGTH5Rzmt9De79ytWWvY320gh8GiT',
    //   bucket: 'jiali-bucket'
    // });
...
export async function uploadFile(option = {}, config) {
    try {
        ...
        //调用 oss实例put方式上传文件
        const result = await clientOSS.put(ossObj, OSS.Buffer(fileResult));
        result.url = result.url.replace('http://jiali-bucket.oss-cn-beijing.aliyuncs.com', 'https://oss.jealee.shop')
        return option.onSuccess ? option.onSuccess(result) : result
    } catch (e) {
        return option.onError ? option.onError('上传失败') : { res: 'FAIL' }
    }
