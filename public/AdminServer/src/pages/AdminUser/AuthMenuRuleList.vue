    ...
    <div class="project-top">
      <el-form :inline="true" class="demo-form-inline">
        <el-form-item label="菜单名称:">
          <el-input clearable placeholder="菜单名称" v-model="name"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="SearchClick()">搜索</el-button>
          <el-button class="pull-right" type="primary" @click="AddUserInfoShow()">添加菜单</el-button>
        </el-form-item>
      </el-form>
    ...
    width="50%">
      <el-form :model="FormData" :rules="rules" ref="FormData" label-width="80px">
          ...
          <el-form-item label="icon">
              <el-input v-model="FormData.icon" placeholder="请输入icon 例如：el-icon-user-solid 参考Element UI"></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    getListData(ServerWhere) {
      ...
      get("/Admin/Authmenurule/getAuthMenuRuleList", ServerWhere, json => {
    ...
    DelData(row) { //删除管理员
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
            ...
            get('/Admin/Authmenurule/delAuthMenuRuleRow', {id: row.id}, (json) => {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
            ...
            if(this.is_save_add_start == 1) {
                var url = "/Admin/Authmenurule/addAuthMenuRuleData";
            } else {
                var url = "/Admin/Authmenurule/saveAuthMenuRuleData";
            }
    ...
    startKeywordList(row) { //更改状态
        get('/Admin/Authmenurule/startAuthMenuRule', {id: row.id, start: row.status}, (json) => {
            if(json.data.code == 10000) {
                this.$message({
                    message: '修改成功',
                    type: 'success'
