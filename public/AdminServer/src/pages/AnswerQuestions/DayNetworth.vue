<template>
    <div class="app-container">
        <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item to="">净值配置</el-breadcrumb-item>
        </el-breadcrumb>
        <div class="main">
            <el-descriptions :column="3" :title="date">
...
export default {
    ...
    methods: {
        submitForm(formName) {
            ...
            if (valid) {
                post("/Admin/Product/saveDayNetworth", {
        ...
        getDayAmount() { //获取最新数值
            get("/Admin/Product/getNewsBuyAmount", {
                product_id: this.product_id,
            }, json => {
                console.log(json);
                if (json.data.code == 10000) {
                    this.date = json.data.data.date;
                    this.count_balance = json.data.data.count_balance;
                    this.yest_count_balance = json.data.data.yest_count_balance;
                    this.count_buy_number = json.data.data.count_buy_number;
                    // this.count_buy_networth = json.data.count_buy_networth;
                    this.today_net_worth = json.data.data.today_net_worth;
                    this.yest_net_worth = json.data.data.yest_net_worth;
                    this.is_networth = json.data.data.is_networth;
                } else {
                    this.$message.error("加载数据失败");
                }
            });
        ...
        calcNewsNetWorth(amount) { //实时根据利润值计算当天净值
            ...
            if(amount && amount !== 0) {
                ...
                get("/Admin/Product/calcNewsNetWorth", {
                    address: this.address,
                    profit: amount
                }, json => {
                    if (json.data.code == 10000) {
                        this.newDaynetworth = json.data.data;
                    } else {
