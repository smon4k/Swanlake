<template>
  <div>
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item to="/home/ticket/list">门票列表</el-breadcrumb-item>
      <el-breadcrumb-item to="">门票明细</el-breadcrumb-item>
    </el-breadcrumb>
    <div>门票：{{ ticketName }}</div>
    <br />
    ...
    <el-table height="500" :data="tableData.list" style="width: 100%" v-loading="loading">
      ...
      <el-table-column prop="count_watch_bonus" label="观看奖励" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.count_watch_bonus }} H2O</span>
        </template>
      </el-table-column>
      <el-table-column prop="count_notes_award" label="笔记奖励" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.count_notes_award }} H2O</span>
        </template>
      </el-table-column>
      <el-table-column prop="count_watch_notes" label="奖励合计" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.count_watch_notes }} H2O</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getListData(ServerWhere) {
      ...
      get("/Admin/Ticket/getTicketDetailsList", ServerWhere, (json) => {
        console.log(json);
        this.loading = false;
        if (json.data.code == 10000) {
    ...
    //   console.log(this.date);
      ...
      if (this.ticketId && this.ticketId !== "") {
        SearchWhere["ticket_id"] = this.ticketId;
      }
