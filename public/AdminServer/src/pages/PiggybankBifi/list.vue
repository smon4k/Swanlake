    ...
    <el-collapse v-model="activeNames" @change="handleChange" v-for="(item, index) in tableData" :key="index">
        <el-collapse-item :title="item.time" :name="index">
            <template slot="title">
                ...
                <div style="margin-right:100px;">价差：{{ keepDecimalNotRounding(item.price, 4, true) }} BUSD</div>
                <div>利润：{{ keepDecimalNotRounding(item.profit, 8, true) }} BUSD</div>
            </template>
                ...
                <el-table-column prop="profit" label="利润" align="center" width="150">
                    <template slot-scope="scope">
                      <div v-if="scope.row.profit">
                        <span>{{ keepDecimalNotRounding(scope.row.profit, 8, true) }} BUSD</span>
                      </div>
                      <div v-else>——</div>
                    </template>
                </el-table-column>
...
export default {
  ...
  methods: {
    getListData(ServerWhere) {
      ...
      get("/Admin/Binancepiggybank/getPiggybankOrderList", ServerWhere, json => {
          console.log(json);
