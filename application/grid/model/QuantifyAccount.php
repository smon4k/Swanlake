...
class QuantifyAccount extends Base
{
    ...
    */
    ...
    public static function getMaxMinUplRateData($account_id=0, $currency='', $page, $limits=0) {
        ...
        $count_sql = "SELECT DISTINCT `trade_id`, MAX(`date`) as latest_date FROM g_quantify_account_positions_rate WHERE `account_id` = {$account_id} GROUP BY `trade_id`";
        ...
        $sql = "SELECT * FROM g_quantify_account_positions_rate WHERE `account_id` = {$account_id} AND `trade_id` IN ({$tradeIdStr}) AND `date` IN (
            SELECT MAX(`date`) FROM g_quantify_account_positions_rate WHERE `account_id` = {$account_id} AND `trade_id` IN ({$tradeIdStr}) GROUP BY `trade_id`
