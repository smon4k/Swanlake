...
use app\grid\model\QuantifyAccount;
...
class QuantifyaccountController extends BaseController
{
    ...
    */
    ...
    public function addQuantityAccount(Request $request) {
        ...
        $exchange = $request->post('exchange', '', 'trim');
        $is_position = $request->post('is_position', 0, 'intval');
        if ($api_key == '' || $name == '' || $secret_key == '' || $pass_phrase == '' || $exchange == '') {
            return $this->as_json('70001', 'Missing parameters');
        }
        $result = Accounts::addQuantityAccount($name, $api_key, $secret_key, $pass_phrase, $exchange, $is_position);
        if($result) {
            return $this->as_json($result);
        } else {
