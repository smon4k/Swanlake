...
class MyProduct extends Base {
            ...
            try {
                if($userId) {
                    ...
                    if ($myProduct && count((array)$myProduct) > 0) { //已存在 进行更新数据
                        ...
                        $res = self::name('a_my_product')->where('id', $myProduct['id'])->update($update);
                    } else {  //不存在 添加
                        if($type == 2) {
                            return false;
                        }
                        ...
                        self::name('a_my_product')->insert($insertData);
    ...
    public static function getMyProductList($where, $page, $limit, $order='id desc', $userId=0)
    {
        ...
        $count = self::name('a_my_product')->alias('a')->join('s_product b', 'a.product_id = b.id')->where($where)->count();//计算总页面
        $allpage = intval(ceil($count / $limit));
        $lists = self::name('a_my_product')->alias('a')
    ...
    public static function getNewsBuyAmount($product_id=0, $userId=0, $iscalcToday=true, $toDayDate='', $yestDayDate='') {
        if($product_id > 0 || $userId > 0) {
            $sql = "SELECT sum(total_number) AS count_total_number FROM s_a_my_product WHERE 1=1 ";
            if($product_id) {
    ...
    public static function getAllProductBalance($uid=0) {
        if($uid > 0) {
            $sql = "SELECT product_id,uid,total_number FROM s_a_my_product WHERE uid={$uid}";
            $data = self::query($sql);
            $count_balance = 0;
    ...
    public static function getUserTotalInvest($userId=0, $product_id=0) {
        $sql = "SELECT sum(total_invest) AS total_invest, sum(total_number) AS total_number FROM s_a_my_product WHERE 1=1 ";
        if ($userId && $userId > 0) {
            $sql .= " AND uid={$userId}";
        }
    ...
    public static function getMyProduct($product_id=0, $uid=0) {
        if($product_id > 0 && $uid > 0) {
            $res = self::name('a_my_product')->where(['product_id'=>$product_id, 'uid'=>$uid])->find();
    ...
    public static function saveUserProductData($date='', $yestDate='', $where=[]) {
        $data = self::name('a_my_product')->where($where)->select()->toArray();
        if($date == '' || $yestDate == '') {
            $date = date('Y-m-d');
            $yestDate = date('Y-m-d', strtotime("-1 day"));
        }
    ...
    public static function getAllCumulativeIncome($userId=0) {
        if($userId > 0) {
            ...
            $data = self::name('a_my_product')->where('uid', $userId)->select()->toArray();
            if($data && count((array)$data) > 0) {
                foreach ($data as $key => $val) {
                    $NewTodayYesterdayNetworth = DayNetworth::getNewTodayYesterdayNetworth($val['product_id']);
                    $toDayNetworth = (float)$NewTodayYesterdayNetworth['toDayData']; //今日最新净值
    ...
    public static function getSumProductTotalInvest($product_id=0) {
        $num = 0;
        if($product_id) {
            $num = self::name('a_my_product')->where('product_id', $product_id)->sum('total_invest');
        }
        return $num;
    }
}