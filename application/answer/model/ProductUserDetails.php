...
class ProductUserDetails extends Base {
    ...
    public static function getMyProductDetailsList($where, $page, $limit, $order='date desc')
    {
        ...
        $count = self::name('a_product_user_details')->alias('a')->where($where)->count();//计算总页面
        $allpage = intval(ceil($count / $limit));
        $lists = self::name('a_product_user_details')->alias('a')
                    ->field("a.*")
                    ->where($where)
                    ->page($page, $limit)
                    ->order($order)
    ...
    public static function setProductDetails($data=[]) {
        if($data) {
            try {
                $date = date('Y-m-d');
                $IsResData = self::name('a_product_user_details')->where(['product_id'=>$data['product_id'], 'uid' => $data['uid'], 'date' => $date])->find();
                if($IsResData && count((array)$IsResData) > 0) {
                    ...
                    $res = self::name('a_product_user_details')->where(['product_id'=>$data['product_id'], 'uid' => $data['uid'], 'date' => $date])->update($updateData);
                    if($res) {
                        return true;
                    }
                } else {
                    self::name('a_product_user_details')->insert($data);
                    $insertId = self::name('a_product_user_details')->getLastInsID();
                    if($insertId) {
                        return true;
                    }
                }
    ...
    public static function getAverageDailyRate($product_id=0, $uid=0, $todayRate=0) {
        if($product_id > 0 && $uid > 0) {
            ...
            $data = self::name('a_product_user_details')->where(['product_id' => $product_id, 'uid' => $uid])->field('daily_rate_return')->select()->toArray();
