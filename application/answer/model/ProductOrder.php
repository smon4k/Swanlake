...
class ProductOrder extends Base {
    ...
    public static function getProductOrderList($where, $page, $limit, $order='date desc')
    {
        ...
        $count = self::name('a_product_order')->alias('a')->join('s_product b', 'a.product_id=b.id')->where($where)->count();//计算总页面
        $allpage = intval(ceil($count / $limit));
        $lists = self::name('a_product_order')->alias('a')
                    ->field("a.*")
    ...
    public static function setProductOrder($userId='', $product_id=0, $quantity=0, $number=0, $networth=0, $type=0, $status=1) {
        if($userId > 0 && $product_id > 0 && $quantity > 0 && $number > 0 && $networth > 0) {
            try {
                ...
                self::name('a_product_order')->insert($insertData);
                $insertId = self::name('a_product_order')->getLastInsID();
                if($insertId) {
                    return true;
                }
                return false;
