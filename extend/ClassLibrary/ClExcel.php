...
class ClExcel {
     ...
     * @return string
     */
    public static function exportToCsv($titles, $values = [], $delimiter = ",", $enclosure = '"', $escape_char = "\\") {
        $file = DOCUMENT_ROOT_PATH . '/temp.csv';
        array_unshift($values, $titles);
        $f = fopen($file, 'w+');
        foreach ($values as $each_array) {
            foreach($each_array as $t){
     ...
     * @throws \PhpOffice\PhpSpreadsheet\Writer\Exception
     */
    public static function ExcalToCsv($csv_file, $suffix = 'csv', $is_delete = false, $file_type="xlsx") {
        if (!is_file($csv_file)) {
            le_info(sprintf('文件“%s”，不存在。', $csv_file));
            return false;
        }
        ...
        $spreadsheet_from_csv = $reader->load($csv_file);
        $suffix               = strtolower($suffix);
        $writer = IOFactory::createWriter($spreadsheet_from_csv, 'Csv');
        $excel_file                         = explode('.', $csv_file);
        $excel_file[count($excel_file) - 1] = $suffix;
        $excel_file                         = implode('.', $excel_file);
        //保存excel
